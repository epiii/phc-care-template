...
export default class CardPasien extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    render() {
        ...
        return (
            <View style={{flexDirection: 'row', width: '95%', alignSelf: 'center' }}>
                <Image style={{width: 60, height: 60, borderRadius: 50}} source={this.props.pic}/>
                <View style={{marginLeft: 20}}>
                    <Text style={styles.h0}>{this.props.nama}</Text>
                    <Text style={styles.h2}>{this.props.norm}</Text>
                    <Text style={styles.h2}>{this.props.kota}</Text>
                </View>                           
            </View>
        );
    }
